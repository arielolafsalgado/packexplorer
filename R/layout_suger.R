...
layout_suger <- function(gs,suggested,gd=NULL,recenter.suggested=TRUE){
	...
	if(!recenter.suggested){
        ...
        vs = max(stats::var(l0[isolates,1]),l0[isolates,2])
        vc = max(stats::var(l0[!isolates,1]),l0[!isolates,2])
        if(sum(isolates)>0){
            for(i in 1:sum(isolates)){
                r = stats::runif(n = 1,min = 0,max = min(vs,vc,na.rm=TRUE)/10)
                phi = stats::runif(n=1,min=0,max=2*pi)
                l[i,1] = r*cos(phi)+xs
                l[i,2] = r*sin(phi)+ys
            }
        }
